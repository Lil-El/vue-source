...
// yxd 笔记：
/**
 * template使用了computed的属性；
 * 渲染watcher执行的时候，Dep.target为渲染watcher，此时deps=[render watcher]；
 * 然后执行渲染函数
 * 其中又使用了computed的值，computed watcher一执行，Dep.target为computed watcher；deps=[render watcher,computed watcher]
 * computed 执行后，deps数组pop，Dep.target为deps的最后一个，即渲染watcher
 * 继续向下执行后，deps再pop，Dep.target为空了
 */
